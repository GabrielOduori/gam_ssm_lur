"""Step 4 — Evaluation: metrics, LOOCV, calibration, diagnostic figures."""
...
def evaluate_model(model, temporal, static, hybrid_pred, epa_eval, args,
    ...
    logger.info("GAM only  (vs EPA, T×S): RMSE=%.3f  MAE=%.3f  R²=%.3f",
                gam_temporal_metrics.rmse, gam_temporal_metrics.mae, gam_temporal_metrics.r2)
    logger.info("GAM+SSM   (vs EPA, T×S): RMSE=%.3f  MAE=%.3f  R²=%.3f",
                hybrid_metrics.rmse, hybrid_metrics.mae, hybrid_metrics.r2)
    ...
    for sid, grp in epa_eval.groupby("station_id"):
        ...
        r2_s = (float(np.corrcoef(y_s, yh_s)[0, 1] ** 2)
    ...
    logger.info("Site-specific temporal R² — median across %d stations: %.3f",
                len(temporal_r2_df), float(temporal_r2_df["temporal_r2"].median()))
    ...
    logger.info("Temporal mean vs Atmos-plan annual: RMSE=%.3f  MAE=%.3f  MBE=%.3f  R²=%.3f",
                atmos_metrics.rmse, atmos_metrics.mae, atmos_metrics.mbe, atmos_metrics.r2)
    ...
    logger.info("Atmos-plan deviation saved (mean=%.3f)",
    ...
    station_annual[args.point_obs_value_col] = station_annual["grid_id"].map(atmos_target)
    ...
    if len(station_annual) >= 2:
        ...
        cv_df = evaluator.loocv_stations(station_annual, obs_col=args.point_obs_value_col)
        cv_df.to_csv(output_dir / "loocv_results.csv", index=False)
        cv_metrics = evaluator.compute_accuracy(cv_df["obs_no2"].values, cv_df["no2"].values)
        logger.info("LOOCV: RMSE=%.3f  MAE=%.3f  R²=%.3f",
                    cv_metrics.rmse, cv_metrics.mae, cv_metrics.r2)
...
def _write_comparison_table(gam_spatial_metrics, gam_temporal_metrics,
                             hybrid_metrics, loocv_metrics, calibration,
                             output_dir: Path):
    ...
    df = pd.concat([df.reset_index(), pd.DataFrame([{
        "Model": "Improvement (GAM→SSM)",
        "RMSE":           round(float(gam_row["RMSE"]) - float(ssm_row["RMSE"]), 3),
