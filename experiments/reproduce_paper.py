...
def run(args) -> None:
    ...
    logger.info("Data loaded: %d cells, %d dates, %d dense-obs rows, %d station rows",
                len(static.grid_ids), len(temporal.dates),
                len(temporal.dense_obs), len(temporal.point_obs))
    logger.info("Calibration: β₀=%.3f  β₁=%.3f  r=%.3f", cal.beta0, cal.beta1, cal.r)
    logger.info("STEP 1 done in %s", _elapsed(t_stage)); t_stage = time.time()
    ...
    feat_df, y_full = filter_sparse_cells(
        feat_df, static.target[args.target_col],
        min_nonzero_features=args.min_nonzero_features, id_cols=ID_COLS,
    )
    ...
    logger.info("STEP 2 done in %s", _elapsed(t_stage)); t_stage = time.time()
    ...
    logger.info("SSM: converged=%s  iterations=%d  LL=%.4e  mode=%s",
                ssm_diag.em_converged, ssm_diag.em_iterations,
                ssm_diag.log_likelihood, model.ssm_.kf_.mode)
    logger.info("SSM fitting:  %s", _elapsed(t_ssm))
    model.save(output_dir / "model")
    logger.info("STEP 3 done in %s", _elapsed(t_stage)); t_stage = time.time()
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", "-o", default=str(EXPERIMENTS / "results" / "reproduce"),
    ...
    if "--output-dir" not in sys.argv and "-o" not in sys.argv:
        ...
        args.output_dir = str(EXPERIMENTS / "results" / f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
