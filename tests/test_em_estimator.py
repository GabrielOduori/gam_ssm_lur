...
class TestEMNumericalStability:
    ...
    def test_dynamic_dim_holds_static_block_at_zero_process_noise(self):
        ...
        for t in range(1, T_len):
            alpha[t] = true_T_dyn * alpha[t - 1] + np.random.normal(0, np.sqrt(true_Q_dyn))
        ...
        em = EMEstimator(max_iterations=50, tolerance=1e-7, verbose=False, dynamic_dim=1)
    ...
    def test_time_varying_Z_with_regression_effect_recovers_truth(self):
        ...
        em = EMEstimator(max_iterations=80, tolerance=1e-8, verbose=False, dynamic_dim=1)
        result = em.fit(observations, state_dim=2, obs_dim=1, Z=Z_seq, scalability_mode='dense')
    ...
    def test_stabilize_transition_clips_explosive_estimate(self):
        ...
        em = EMEstimator(max_iterations=30, tolerance=1e-6, verbose=False, max_eigenvalue=0.98)
        result = em.fit(observations, state_dim=state_dim)
