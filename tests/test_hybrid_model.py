...
def generate_synthetic_data(
    ...
    y = (
        spatial_effect
        + temporal_expanded
        + np.random.randn(n_locations * n_times) * noise_level
    )
...
class TestHybridGAMSSM:
    ...
    def test_evaluate(self):
        ...
        predictions = model.predict()
        metrics = model.evaluate(model._y_matrix.flatten(), predictions.total.flatten())
    ...
    def test_save_load(self, tmp_path):
        ...
        np.testing.assert_array_almost_equal(pred_original.total, pred_loaded.total)
